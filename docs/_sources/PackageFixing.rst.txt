...
Note that the metadata at the end of the description is what gets appended when
filing a bug report is automatically triggered, or if the user uses a bug
reporting assistant (i.e. Apport).
...
you can (examine it manually)[DebugApportCrash.md] to get a useful stacktrace.


Try to reproduce the issue
--------------------------

Not all bugs can be easily reproduced, and not all reproducible bugs will be
...
enough to identify a fix.

However, in this case we're lucky.  The bug triagers have identified a way to
reproduce the issue, in `comment #12`_::
...
.. _comment #12: https://bugs.launchpad.net/ubuntu/+source/postfix/+bug/1753470/comments/12

Let's see if we can reproduce the issue as well, using these directions.

Before that, we need to set up an environment for doing the testing.  There's
many options for where to do your testing, and different developers have their
own preferences.  Here's a couple:
...
Often we can save time by leveraging someone else's work, so it's always worth
doing some research upfront.  Fixes for bugs can be found in newer versions of
Ubuntu, Debian, or upstream, and sometimes in external forums or bug trackers.
...
We see that 3.3.0-1ubuntu1 exists under cosmic, so postfix has been modified
there. Let's see what was changed.
...
This will create a new git clone of the postfix repo named "postfix-gu", with a
...
* Due to https://launchpad.net/bugs/1761821, you may get: ``fatal: could not
  read Username for 'https://git.launchpad.net': terminal prompts disabled.``
  It's safe to ignore this.

* First time will add a git-ubuntu entry to .gitignore

* Sometimes it can also be helpful to checkout the git repositories for the
  package maintained by Debian and/or upstream.  These would be checked out to
  "postfix-debian" and "postfix" respectively.
...
Here we see the patch and the change to debian/patches/series to include the
patch. This is the fix we need!
...
for many (but not all) of its packages, so having a clone of this can be handy.

For example, let's assume for argument's sake that we had a problem with sshd
in xenial, where it would fail to check config files before reloading
(https://bugs.launchpad.net/ubuntu/+source/openssh/+bug/1771340).  From
Debian's openssh source package page
(https://packages.debian.org/source/stretch/openssh), we find the git
repository (https://salsa.debian.org/ssh-team/openssh), and can check it out::
...
https://salsa.debian.org.  Doing so in this case would bring you to
...
Since we can't push new versions of packages to previous Ubuntu releases, you'd
...
This only works if the package has a debian/watches file.  If it doesn't, look
in the package's README or other documentation, and do the research online
manually.

Searching the upstream bug tracker, or generally googling on error messages or
...
Each upstream project has its own conventions and expectations for how they can
...
contributions for best practices to follow.
...
simplify it down to the minimal set of commands to reproduce the problem.

When filing the bug report or pull request upstream, do identify yourself as a
...
Changes to packages are done via patches. The patches themselves are stored in
...
what changes have been made to the package over time.
...
don't see yellow pencil icons, you need to get permissions.
...
``bionic-devel``. It helps to use a branch name that's descriptive::
...
If the only changes you made are within the debian subdir, you don't need a
patchfile, and can skip this step.
...
See :doc:`CommittingChanges`.


Build a Fixed Package
=====================

See :doc:`PackageBuilding`.
...
reuse our git branch name, and just prefix the package name::
...
In this case, I'm using the PPA. Alternatively, if you've built locally, you
can copy in the .deb file and install it manually.
...
The DEP8 autopkgtests don't exercise our bug, but are worth running as
just-in-case sanity checks and to catch regressions.

See :doc:`PackageTests`.

Any change in behavior should be considered priorities to resolve before
proceeding.
...
Go back to the bug report (in my case,
https://bugs.launchpad.net/ubuntu/+source/postfix/+bug/1753470).

Modify the bug description (yellow pencil icon) and update it to conform with
https://wiki.ubuntu.com/StableReleaseUpdates#SRU_Bug_Template  These are
...
for addressing (or skipping) it in LTS and other stable releases.

Note: Keep the original description as-is, in a section called ``[Original
Description]`` at the bottom.

Note: You'll see your branch and merge proposal in the ``Related branches``
because of the (LP: #NNNNNN) in the changelog entry.
...
* The diff fully explained by the changelog entry. This means that if there is
  something in the diff that is not explained by the changelog, then there is a
  problem.

* A bug for everything mentioned in the changelog entry. Reviewers are
...
* Every issue mentioned in an SRU changelog must have a bug task filed against
  ...
  the server team can assist if you need help creating bug tasks.
...
* Every LP bug # mentioned in an SRU changelog must have "SRU paperwork" filled
...
* The upload will appear in the "unapproved queue", for example
  https://launchpad.net/ubuntu/focal/+queue?queue_state=1.  It may take a week
  or two before its processed.

* If you find a problem while its still unapproved, ask in the Freenode
  ...
  same version number can be re-used in a subsequent upload.
...
(https://launchpad.net/ubuntu/+source/postfix/3.3.0-1ubuntu0.1) and make sure
...
Eventually, the package with your fixes will appear there (search for postfix
in this case). It will show the dep8 tests for postfix and anything that
depends on it. Any tests that fail will show in red.
...
https://people.canonical.com/~ubuntu-archive/pending-sru.html shows what SRUs
...
update, so it's wise to followup if this happens.

Once all of the SRU's bugs have reached verification-done and a 7-day waiting
period has elapsed, the SRU team will move the source and binary packages into
the -updates pocket and mark the bug task(s) as Fix Released.
